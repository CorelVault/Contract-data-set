...
contract Prometheus is ERC721A, Ownable, ERC2981, DefaultOperatorFilterer {
    using Strings for uint256;
    ...
    uint256 public wl_per_wallet = 1;    
    uint256 public total_wl_limit = 15;
    uint256 public PUBLIC_MAX_SUPPLY = 555;  
    uint256 public total_wl_count;    

    uint256 startTime = 1701457200; // Dec 1 2023, 19:00

    mapping(address => uint256) public publicSaleMinted;
    mapping(address => uint256) public wlMinted;
    mapping(address => bool) public wlEligible;

    constructor(string memory _initBaseURI, string memory _initNotRevealedUri,
     string memory _contractURI, address[] memory wlAddresses)
     ERC721A("Prometheus", "Prometheus") {
    ...
    mint(1);
    whitelistAddresses(wlAddresses);
    }
    ...
    function mint(uint256 quantity) public payable  {
        require((totalSupply()) + quantity <= PUBLIC_MAX_SUPPLY,"No More NFTs to Mint");
        
        if (msg.sender != owner()) {

            require(startTime <= block.timestamp, "Minting not yet started");
            require(public_mint_status, "Public mint status is off");           
            require(wlMinted[msg.sender] + publicSaleMinted[msg.sender] + quantity <= max_per_wallet, "Per Wallet Limit Reached");
    ...
    function whitelistMint(uint256 quantity) payable public{

        require(wlEligible[msg.sender], "Not eligible for whitelist");
   
        require(totalSupply() + quantity <= PUBLIC_MAX_SUPPLY,"No More NFTs to Mint");
        require(wl_mint_status, "whitelist mint is off");
        require(total_wl_count + quantity <= total_wl_limit, "Whitelist Limit Reached");        
        require(wlMinted[msg.sender] + quantity <= wl_per_wallet, "Whitelist Limit per Wallet Reached");  
        require(wlMinted[msg.sender] + publicSaleMinted[msg.sender] + quantity <= max_per_wallet, "Per Wallet Limit Reached");
    ...
    function whitelistAddresses(address[] memory wl) public onlyOwner{
        for(uint256 x = 0; x < wl.length; x++){
        wlEligible[wl[x]] = true;
        }
    }
    ...
    function toggle_wl_mint_status() external onlyOwner {
    ...
    function setWl_per_wallet(uint256 _wl_per_wallet) external onlyOwner {
        wl_per_wallet = _wl_per_wallet;
    }

    function setTotal_wl_limit(uint256 _total_wl_limit) external onlyOwner {
        total_wl_limit = _total_wl_limit;
    }
 

    function setPUBLIC_MAX_SUPPLY(uint256 _PUBLIC_MAX_SUPPLY) external onlyOwner {
        PUBLIC_MAX_SUPPLY = _PUBLIC_MAX_SUPPLY;
    }
