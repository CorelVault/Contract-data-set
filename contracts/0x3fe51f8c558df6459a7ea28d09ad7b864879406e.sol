...
Website: https://kakegurui.fun
Telegram: https://t.me/kakeguruieth
X: https://twitter.com/KakeguruiERC20
...
𝕁𝕒𝕓𝕒𝕞𝕚 𝕐𝕦𝕞𝕖𝕜𝕠 𝕚𝕤 𝕟𝕠𝕥 𝕛𝕦𝕤𝕥 𝕒 𝕔𝕙𝕒𝕣𝕒𝕔𝕥𝕖𝕣; 𝕤𝕙𝕖'𝕤 𝕒 𝕤𝕪𝕞𝕓𝕠𝕝 𝕠𝕗 𝕣𝕚𝕤𝕜 𝕒𝕟𝕕 𝕣𝕖𝕨𝕒𝕣𝕕,
𝕖𝕔𝕙𝕠𝕚𝕟𝕘 𝕥𝕙𝕖 𝕖𝕤𝕤𝕖𝕟𝕔𝕖 𝕠𝕗 𝕥𝕙𝕖 𝕔𝕣𝕪𝕡𝕥𝕠 𝕝𝕒𝕟𝕕𝕤𝕔𝕒𝕡𝕖, 𝕨𝕙𝕖𝕣𝕖 𝕣𝕚𝕤𝕜𝕤 𝕒𝕟𝕕 𝕣𝕖𝕨𝕒𝕣𝕕𝕤 𝕕𝕒𝕟𝕔𝕖 𝕚𝕟 𝕒𝕟 𝕚𝕟𝕥𝕣𝕚𝕔𝕒𝕥𝕖 𝕥𝕒𝕟𝕘𝕠. 
ℍ𝕖𝕣 𝕤𝕡𝕚𝕣𝕚𝕥, 𝕙𝕖𝕣 𝕝𝕠𝕧𝕖 𝕗𝕠𝕣 𝕔𝕙𝕒𝕝𝕝𝕖𝕟𝕘𝕖𝕤 𝕒𝕟𝕕 𝕥𝕙𝕖 𝕥𝕙𝕣𝕚𝕝𝕝 𝕠𝕗 𝕥𝕙𝕖 𝕘𝕒𝕞𝕖, 𝕣𝕖𝕗𝕝𝕖𝕔𝕥𝕤 𝕥𝕙𝕖 𝕖𝕟𝕥𝕙𝕦𝕤𝕚𝕒𝕤𝕞 𝕒𝕟𝕕 𝕥𝕙𝕖 𝕣𝕚𝕤𝕜𝕤 𝕒𝕤𝕤𝕠𝕔𝕚𝕒𝕥𝕖𝕕 𝕨𝕚𝕥𝕙 𝕥𝕙𝕖 𝕣𝕒𝕡𝕚𝕕𝕝𝕪 𝕖𝕧𝕠𝕝𝕧𝕚𝕟𝕘 𝕔𝕣𝕪𝕡𝕥𝕠 𝕞𝕒𝕣𝕜𝕖𝕥.
*/
...
contract YUMEKOJABAMI  is ERC20, Ownable {
    ...
    constructor() ERC20("YUMEKO JABAMI", "KAKEGURUI") {
        ...
        MarketingAddress = address(0x2a77e5D19fdDc6a874a774a5F444aA125eba3844);
    ...
    function updateMaxBuyAmount(uint256 newAmount) external onlyOwner {
        require(newAmount * 1e18 >= (totalSupply() * 1 / 100), "KAKEGURUI: new max buy amount less than 1% of total supply");
        maxBuyAmount = newAmount * 1e18;
    }

    function updateMaxSellAmount(uint256 newAmount) external onlyOwner {
        require(newAmount * 1e18 >= (totalSupply() * 1 / 100), "KAKEGURUI: new max sell amount less than 1% of total supply");
        maxSellAmount = newAmount * 1e18;
    }

    function updateMaxWalletAmount(uint256 newAmount) external onlyOwner {
        require(newAmount * 1e18 >= (totalSupply() * 2 / 100), "KAKEGURUI: new max wallet amount less than 2% of total supply");
        maxWalletAmount = newAmount * 1e18;
    }

    function updateSwapTokensAtAmount(uint256 newAmount) external onlyOwner {
        require(newAmount * 1e18 >= totalSupply() * 1 / 100000, "KAKEGURUI: new swap amount less than 0.001% of total supply");
        require(newAmount <= totalSupply() * 1 / 1000, "KAKEGURUI: new Swap amount exceeds 0.1% of total supply");
    ...
    function excludeFromLimits(address account, bool isExcluded) external onlyOwner {
        if (!isExcluded) {
            require(account != uniswapV2Pair, "KAKEGURUI: account to be exscluded is pair address");
        }
        _excludeFromLimits(account, isExcluded);
    }

    function updateBuyFee(uint256 newBuyFee) external onlyOwner {
        require(newBuyFee <= 5, "KAKEGURUI: new buy fee exceeds 5%");
        buyFee = newBuyFee;
    }

    function updateSellFee(uint256 newSellFee) external onlyOwner {
        if (sellFee > 5) {
            require(newSellFee <= sellFee, "KAKEGURUI: new sell fee exceeds current sell fee");
        } else {
            require(sellFee <= 5, "KAKEGURUI: new sell fee exceeds 5%");
        }
        sellFee = newSellFee;
    }
    ...
    function _transfer(address from, address to, uint256 amount) internal override {
        require(amount > 0, "KAKEGURUI: amount is zero");
        require(!isSniper[from], "KAKEGURUI: sender marked as sniper");
        require(!isSniper[to], "THOKAKEGURUIT: recipient marked as sniper");
 
        if (limitsInEffect) {
            if (from != owner() && to != owner() && !isExcludedFromFees[from] && !isExcludedFromFees[to]) {
                require(tradingActive, "KAKEGURUI: trading is not active");
                ...
                if (automatedMarketMakerPairs[from] && !isExcludedFromLimits[to]) {
                    require(amount <= maxBuyAmount, "KAKEGURUI: transfer amount exceeds max buy amount");
                    require(amount + balanceOf(to) <= maxWalletAmount, "KAKEGURUI: balance exceeds max wallet amount");
                }
                // sell
                else if (automatedMarketMakerPairs[to] && !isExcludedFromLimits[from]) {
                    require(amount <= maxSellAmount, "KAKEGURUI: transfer amount exceeds max sell amount");
                }
    ...
    function setAutomatedMarketMakerPair(address pair, bool isAMM) external onlyOwner {
        if(!isAMM) {
            require(pair != uniswapV2Pair, "KAKEGURUI: automated market maker pair is uniswap V2 pair");
        }
    ...
    function enableTrading(bool _status, uint256 _deadBlocks) external onlyOwner {
        require(!tradingActive, "KAKEGURUI: trading is already active");
        require(_deadBlocks <= 5, "KAKEGURUI: deadblocks amount exceeds 5");
    ...
    function setMarketingAddress(address newMarketingAddress) external onlyOwner {
        require(newMarketingAddress != address(0), "KAKEGURUI: new marketing address is the zero address");
        MarketingAddress = payable(newMarketingAddress);
    }

    function manageSniper(address sniperAddress, bool status) external onlyOwner {
        require(sniperManagementEnabled, "KAKEGURUI: sniper management permanently disabled");
        isSniper[sniperAddress] = status;
    }

    function manageSnipers(address[] calldata addresses, bool status) external onlyOwner {
        require(sniperManagementEnabled, "KAKEGURUI: snipers management permanently disabled");
    ...
    function transferForeignToken(address token, address to) external onlyOwner returns (bool) {
        require(token != address(0), "KAKEGURUI: token is the zero address");
